...
contract StorageEntityManagerTest is Test {
    ...
    function test_createStorageEntitySuccess() public {
        ...
        for (uint256 i = 0; i < allowedCallers.length; i++) {
            ...
            Storage.StorageEntity memory storageEntity = roundRobinAllocator.getStorageEntity(se);
            assertEq(storageEntity.owner, se);
            assertEq(storageEntity.storageProviders[0], storageProviders[0]);
            assertTrue(roundRobinAllocator.isStorageProviderUsed(storageProviders[0]));
        }
        Storage.StorageEntity[] memory storageEntities = roundRobinAllocator.getStorageEntities();
    ...
    function test_addStorageProviderSuccess() public {
        Storage.StorageEntity memory storageEntity = _prepareStorageEntity(address(1), new uint64[](0));
        ...
        roundRobinAllocator.addStorageProviders(storageEntity.owner, storageProviders);
        ...
        assertTrue(roundRobinAllocator.isStorageProviderUsed(updatedStorageEntity.storageProviders[0]));
        assertTrue(roundRobinAllocator.isStorageProviderUsed(updatedStorageEntity.storageProviders[1]));
    }
    ...
    function test_removeStorageProvidersSuccess() public {
        ...
        Storage.StorageEntity memory storageEntity = _prepareStorageEntity(address(1), storageProviders);
        assertEq(storageEntity.storageProviders.length, 1);
        ...
        assertFalse(roundRobinAllocator.isStorageProviderUsed(storageProviders[0]));
    }
    ...
    function test_removeStorageProviderMissingStorageProviderSuccess() public {
        ...
        vm.prank(storageEntity.owner);
        roundRobinAllocator.removeStorageProviders(storageEntity.owner, new uint64[](1));
    ...
    function test_changeStorageEntityActiveStatusSuccess() public {
        ...
        vm.prank(storageEntity.owner);
        roundRobinAllocator.changeStorageEntityActiveStatus(storageEntity.owner, true);

        Storage.StorageEntity memory reupdatedStorageEntity = roundRobinAllocator.getStorageEntity(storageEntity.owner);
