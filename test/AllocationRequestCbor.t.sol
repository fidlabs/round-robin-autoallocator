...
contract AllocateFacetTestWrapper is AllocateFacet {
    ...
    function selectors() external pure virtual override returns (bytes4[] memory selectors_) {
        ...
        selectors_[1] = this.encodeAllocations.selector;
    }
...
contract AllocationRequestCborTest is Test {
    AllocateFacetTestWrapper public facet;

    function setUp() public {
        AllocateFacetTestWrapper _facet = new AllocateFacetTestWrapper();

        IFacet[] memory replaceFacets = new IFacet[](1);
        replaceFacets[0] = IFacet(address(_facet));

        address diamond = DiamondDeployer.deployDiamondWithReplace(address(this), replaceFacets);
        facet = AllocateFacetTestWrapper(diamond);
    }

    function _createAllocRequest(bytes memory cid, uint64 size)
        internal
        pure
        returns (Types.AllocationRequest memory)
    {
        return Types.AllocationRequest({dataCID: cid, size: size});
    }

    function test_singleAllocReqSingleReplica() public view {
        Types.AllocationRequest[] memory allocReqs = new Types.AllocationRequest[](1);
        ...
        Types.ProviderAllocationPayload[] memory payloads =
            facet.encodeAllocations(allocReqs, replicaSize, providers, termMin, termMax, expiration);
    ...
    function test_twoAllocReqTwoReplicas() public view {
        Types.AllocationRequest[] memory allocReqs = new Types.AllocationRequest[](2);
        allocReqs[0] = _createAllocRequest("cid1", 100);
    ...
    function test_fiveAllocReqThreeReplicas() public view {
        Types.AllocationRequest[] memory allocReqs = new Types.AllocationRequest[](5);
    ...
    function test_thousandsAllocReqThreeReplicas() public view {
        ...
        Types.AllocationRequest[] memory allocReqs = new Types.AllocationRequest[](allocationsCount);
