...
library AllocationCborTest {
     ...
     */
    function decodeDataArray(bytes memory cborData)
        public
        pure
        returns (Types.AllocationRequestData[] memory requests)
    {
        uint256 byteIdx = 0;
        ...
        requests = new Types.AllocationRequestData[](requestsCount);
        ...
        for (uint256 i = 0; i < requestsCount; i++) {
            ...
            requests[i] = Types.AllocationRequestData(provider, dataCID, size, termMin, termMax, expiration);
        }
